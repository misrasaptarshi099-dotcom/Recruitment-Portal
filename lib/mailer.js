...
/**
 * Small HTML-escaping helper for user-controlled strings in email templates
 */
...
/**
 * Resolves the public recruitment portal URL.
 * Defaults to the live production deployment link requested: https://recruitment-portal-liart.vercel.app/
 * Falls back gracefully if an explicit non-localhost production URL is configured.
 */
export function getPortalUrl() {
  ...
  return "https://recruitment-portal-liart.vercel.app";
}
...
function wrapEmailHtml({ title, preheader, bodyHtml, ctaText, ctaUrl, footerNote }) {
  ...
  const safeTargetCtaUrl = escapeHtml(targetCtaUrl);
  const safeFooterNote = footerNote
    ? escapeHtml(footerNote)
    : "This is an automated notification from Google Developer Groups on Campus (VIT).";
...
<body>
  ...
  <div class="wrapper">
    <div class="card">
      ...
      <div class="content">
        ${bodyHtml}
        ${safeCtaText ? `<a href="${safeTargetCtaUrl}" class="cta-btn" target="_blank">${safeCtaText} &rarr;</a>` : ""}
      </div>
      <div class="footer">
        ${safeFooterNote}
...
export async function sendDecisionEmail({ to, candidateName, department, round, decision, details = {} }) {
  ...
  const dept = escapeHtml(department || "Department");
...
export async function sendInterviewConfirmationEmail({ to, candidateName, department, slotDetails }) {
  ...
  if (!transporter) {
    console.log(`[DRY RUN - INTERVIEW CONFIRMATION] To: ${to}, Dept: ${dept}, Slot: ${slot?.slotLabel}`);
    return { success: true, simulated: true };
  }

  const date = escapeHtml(slot.date || "Scheduled Date");
  const time = escapeHtml(slot.slotTime || slot.slotLabel || (slot.startTime ? `${slot.startTime} - ${slot.endTime}` : "Scheduled Time"));
  const meetLink = slot.meetingLink || slot.meetLink || "";
  ...
  const bodyHtml = `
    ...
    <div class="highlight-box">
      ...
      ${meetLink ? `<strong>Meet Link:</strong> <a href="${safeMeetLink}" style="color:#38bdf8;" target="_blank">${safeMeetLink}</a>` : "<strong>Meet Link:</strong> Available on your candidate portal"}
    </div>
...
export async function sendBatchAnnouncementEmail({ recipients, subject, bodyTemplate }) {
  ...
  for (const recipient of recipients) {
    try {
      ...
      const name = escapeHtml(recipient.Name || recipient.fullName || "Candidate");
      const dept = escapeHtml(recipient.Department || recipient.department || "GDG Department");
