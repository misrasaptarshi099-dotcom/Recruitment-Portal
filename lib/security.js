 ...
 * Only super_admin or dept_manager with at least one department are active.
 * @param {object} entry - { role, departments }
 * @returns {boolean}
 */
export function isActiveAssignment(entry) {
...
export function isUserAdmin(user, roleConfig) {
  ...
  if (roleConfig?.assignments && user.email) {
    const entry = roleConfig.assignments[user.email.trim().toLowerCase()];
    if (isActiveAssignment(entry)) return true;
  }
  return false;
}
