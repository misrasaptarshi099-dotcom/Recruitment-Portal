...
import { isInstitutionalEmail, isAdminEmail, isActiveAssignment } from "./security.js";
...
const firebasePrivateKey = formatPrivateKey(process.env.FIREBASE_PRIVATE_KEY);
...
if (firebaseClientEmail && firebasePrivateKey) {
  try {
    ...
    console.warn("[AUTH] Failed to initialize Firebase credential with cert():", err.message);
  }
}
          ...
          if (user?.email) {
            try {
              ...
              if (snap.exists) {
                ...
                if (config?.assignments && isActiveAssignment(config.assignments[emailLower])) {
                  isAllowed = true;
                  isDynamicAdmin = true;
                }
              }
      ...
      if (ctx.path?.startsWith("/callback")) {
        ...
        if (newSession?.user?.email) {
          ...
          if (!isAllowed) {
            ...
            const tokenCookieName = ctx.context?.authCookies?.sessionToken?.name || "better-auth.session_token";
            if (typeof ctx.setCookie === "function") {
              ctx.setCookie(tokenCookieName, "", { maxAge: 0, path: "/" });
            }
            if (ctx.responseCookies?.delete) {
              ctx.responseCookies.delete(tokenCookieName);
            } else if (ctx.responseCookies?.set) {
              ctx.responseCookies.set(tokenCookieName, "", { maxAge: 0, path: "/" });
            }
